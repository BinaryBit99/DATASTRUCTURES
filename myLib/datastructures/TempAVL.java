...
public class AVL extends BST {
    ...
    private void leftRotation(TNode ancestor, TNode pivot, TNode son ) {
        // TNode temp = pivot;
        if(this.root == ancestor && son.getLeft() == null && son.getRight() == null && pivot.getLeft()==null) {
            this.root = pivot;      // make this.root the parent/pivot node.
            pivot.setParent(null);
            ancestor.setParent(pivot);
            ancestor.setRight(null);
            pivot.setLeft(ancestor);
        }
        else {
            if(pivot != this.root) {
    ...
    private void rightRotation(TNode ancestor, TNode pivot, TNode son) {
        if(this.root == ancestor && son.getLeft() == null && son.getRight() == null && pivot.getRight() == null) {
            this.root = pivot;      // make this.root the parent/pivot node.
            pivot.setParent(null);
            ancestor.setParent(pivot);
            ancestor.setLeft(null);
            pivot.setRight(ancestor);
        }
        else {
            if(pivot != this.root) {
